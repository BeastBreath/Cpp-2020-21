...
int main() {
  student newStudent;
  node* firstNode = new node(&newStudent, NULL);//This is the first node that points to the first one that the user puts in. You don't need a whole node, but its easier and symetrical to other nodes so I like this way better
  firstNode->setNext(NULL);//Sets next node to NULL
  char* input = new char[100];
  cout << "This program creates and stores students, sorting them by Student ID\nValid Commands are:\nHELP   ADD   PRINT   DELETE   AVERAGE   QUIT\n";
...
void ADD(node* currentNode, student* newStudent) {
  ...
  if(currentNode->getNext() == NULL) {
    node* thisNode = new node(newStudent, NULL);
